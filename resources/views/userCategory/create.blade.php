    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('usercategory.store') }}">
                    @csrf
                    <div class="shadow overflow-hidden sm:rounded-md">

                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="cat_name" class="block font-medium text-sm text-gray-700">Categoty</label>
                            <input type="text" name="cat_name" id="cat_name"  class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('cat_name', '') }}" />
                        </div>
                         <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="cat_name" class="block font-medium text-sm text-gray-700">Details</label>
